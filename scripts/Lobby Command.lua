CPPanoramaMainMenu = panorama.loadstring([[
	...
	*/
	...
	// Keep the polling loop inside Panorama. Calling a PersistentProxy from
	// utils.execute_after once per second repeatedly crosses from the plugin
	// worker thread into V8 and eventually corrupts the isolate.
	let PartyChatLoopGeneration = 0;
	let StartPartyChatLoop = ()=>{
		const Generation = ++PartyChatLoopGeneration;
		const Tick = ()=>{
			if ( Generation !== PartyChatLoopGeneration ) return;
			PartyChatLoop();
			$.Schedule(1.0, Tick);
		};
		Tick();
...
local menu=ui.create('Lobby Command','Party Chat',1)
local enabled=menu:switch('Enable lobby commands',true)
local status=menu:label('Open the CS2 main-menu lobby before installing')



menu:button('Install / Restart lobby bot',function ()
    CPPanoramaMainMenu.PartyChatLoop()
end)


-- Enter V8 only once. The recurring loop now runs through Panorama's own
-- $.Schedule and therefore stays on the correct V8/UI thread.
if enabled:get() then
	CPPanoramaMainMenu.PartyChatLoop()
end
...
enabled:set_callback(function()
	if enabled:get() then
		...
		CPPanoramaMainMenu.StopPartyChatLoop()
	end
end)
